...
1. **Contribution Workflow**: All contributions must be submitted via Pull Requests. No direct commits to the main branch are permitted.
2. **Rigid Review**: Every contribution is subject to a mandatory code review by the Governance Authority.
3. **Automated Validation**: The `preflight_check()` within the `validator_agent.py` is an absolute gate. Any contribution failing this check will be automatically rejected.
